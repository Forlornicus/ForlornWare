...
std::map<Closure*, bool> luaF_getclosures() {
    return closureMap;
}
void luaF_clearclosures() {
    closureMap.clear();
}
...
void luaF_replacelclosure(lua_State* L, Closure* closure, Closure* newclosure)
{
    if (closure->isC || newclosure->isC)
    {
        lua_pushstring(L, "Both closures must be a LClosure.");
        lua_error(L);
    }
    closure->env = newclosure->env;
    closure->l.p = newclosure->l.p;
    closure->stacksize = newclosure->stacksize;

    for (int i = 0; i < closure->nupvalues; i++)
        setobj2n(L, &closure->l.uprefs[i], luaO_nilobject);

    int count = min(closure->nupvalues, newclosure->nupvalues);
    for (int i = 0; i < count; i++)
        setobj(L, &closure->l.uprefs[i], &newclosure->l.uprefs[i]);
}

void luaF_replacecclosure(lua_State* L, Closure* closure, Closure* newclosure)
{
    if (!closure->isC || !newclosure->isC)
    {
        lua_pushstring(L, "Both closures must be a C Closure.");
        lua_error(L);
    }

    closure->c.f = newclosure->c.f;
    closure->c.cont = newclosure->c.cont;
    closure->env = newclosure->env;
    closure->stacksize = newclosure->stacksize;
    closure->preload = newclosure->preload;

    for (int i = 0; i < closure->nupvalues; i++)
        setobj2n(L, &closure->c.upvals[i], luaO_nilobject);

    int count = min(closure->nupvalues, newclosure->nupvalues);
    for (int i = 0; i < count; i++)
        setobj2n(L, &closure->c.upvals[i], &newclosure->c.upvals[i]);
}
